module DeviseCrowdAuthenticatable
  class InstallGenerator < Rails::Generators::Base
    ...
    class_option :update_model, :type => :boolean, :default => true, :desc => "Update model to change from database_authenticatable to crowd_authenticatable"
    class_option :add_rescue, :type => :boolean, :default => true, :desc => "Update Application Controller with resuce_from for DeviseCrowdAuthenticatable::CrowdException"
    class_option :advanced, :type => :boolean, :desc => "Add advanced config options to the devise initializer"


    def create_crowd_config
      copy_file "crowd.yml", "config/crowd.yml"
    end
  ...
  # config.crowd_create_user = false
  # config.crowd_update_password = true
  # config.crowd_config = "\#{Rails.root}/config/crowd.yml"
  # config.crowd_check_group_membership = false
